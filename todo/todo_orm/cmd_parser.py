...
class CmdLineParser(ArgumentParser):
    ...
    def execute_sql_by_command(self):
        ...
        if self.dict_args['init']:
            ...
            result = Todo(context=text, id=self.next_id()).save()
            if result:
    ...
    def _print_and_check_result(self, result):
        ...
        if result:
            for r in result:
                if r.update_at == Todo.MAPPINGS['update_at'].default:
                    print(str(r.id) + ' | ' + r.context + 
                         ' (Create at: ' + self.datetime_filter(r.created_at) + ')')
                else:
                    print(str(r.id) + ' | ' + r.context +
                          ' (Create at: ' + self.datetime_filter(r.created_at) + ')' 
                          + ' (Update at: ' + self.datetime_filter(r.update_at) + ')')
        else:
            print('No task exist.')
